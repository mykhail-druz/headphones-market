...
function useParams(params) {
  const stringifiedParams = react.useMemo(() => JSON.stringify(params || null), [params]);
...
const _definePreview = _ref => {
  ...
  return function usePreview(token, query, _params) {
    ...
    return react.useSyncExternalStore(syncStore.subscribe, syncStore.getSnapshot);
  };
};
...
const definePreview = config => _definePreview(_objectSpread(_objectSpread({}, config), {}, {
  ...
  preload: (store, query, params) => suspendReact.suspend(() => _preloadQuery(store, query, params), ["@sanity/preview-kit", "preload", query, JSON.stringify(params != null ? params : null)]),
